...
app.post('/api/save-to-sounds', async (req, res) => {
  ...
  const validFolders = ['done', 'start', 'userpromptsubmit', 'precompact'];
...
app.get('/api/sounds-info', (req, res) => {
  const folders = ['done', 'start', 'userpromptsubmit', 'precompact'];
...
app.post('/api/setup-listener', async (req, res) => {
  ...
  try {
    ...
    if (existsSync(zshrcPath)) {
      zshrcContent = await readFile(zshrcPath, 'utf-8');
      alreadyAdded = zshrcContent.includes('.claude-sounds.zsh');
    }

    // Add to .zshrc if not present
    if (!alreadyAdded) {
      await appendFile(zshrcPath, sourceLine);
      console.log('Added source line to .zshrc');
    }
...
app.get('/api/hooks-status', async (req, res) => {
  ...
  try {
    ...
    const status = {};
    for (const [hookName, hookConfig] of Object.entries(REQUIRED_HOOKS)) {
      const existingHooks = settings.hooks?.[hookName] || [];
      // Check if our touch command exists
      const hasOurHook = existingHooks.some(entry =>
        entry.hooks?.some(h => h.command?.includes('.claude-'))
      );
      status[hookName] = hasOurHook;
    }
...
app.get('/api/listener-status', async (req, res) => {
  ...
  if (existsSync(zshrcPath)) {
    ...
    inZshrc = content.includes('.claude-sounds.zsh');
  }
